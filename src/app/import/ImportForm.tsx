...
const LOCALSTORAGE_KEY = "takumi_external_user_id";

export default function ImportForm({ userId, manualCharts, savedExternalUserId }: ImportFormProps) {
  // --- Direct Import State ---
  const [externalUserId, setExternalUserId] = useState(savedExternalUserId ?? "");
  const [directLoading, setDirectLoading] = useState(false);
  const [directResult, setDirectResult] = useState<ImportResult | null>(null);

  // localStorage からの復元（DBに保存がない場合のみ）
  useEffect(() => {
    if (!savedExternalUserId) {
      try {
        const cached = localStorage.getItem(LOCALSTORAGE_KEY);
        if (cached) setExternalUserId(cached);
      } catch { /* localStorage利用不可の環境対応 */ }
    }
  }, [savedExternalUserId]);
  ...
  const handleDirectImport = async () => {
    ...
    try {
      const importResult = await fetchAndImportScores(userId, externalUserId);
      setDirectResult(importResult);

      // 成功時にlocalStorageにも保存（次回の利便性のため）
      if (importResult.success) {
        try {
          localStorage.setItem(LOCALSTORAGE_KEY, externalUserId.trim());
        } catch { /* 無視 */ }
      }
    } catch (e: unknown) {
  ...
  const renderImportResult = (
    ...
    >
      <div className="flex items-start gap-4">
        ...
        >
          {importResult.success ? <CheckCircle size={24} /> : <AlertTriangle size={24} />}
        </div>
        ...
        <div className="flex-1 space-y-3">
          ...
          {importResult.success ? (
            ...
            <p className="text-[var(--alert-error-text)]">{importResult.error}</p>
          )}
      ...
      <section className="space-y-6">
        <div className="bg-[var(--color-card-bg)] rounded-2xl p-6 border border-[var(--color-header-border)]">
          <div className="flex flex-col items-center text-center space-y-4">
            <div className="w-16 h-16 bg-[var(--color-header-fill,var(--color-accent))] bg-opacity-10 rounded-full flex items-center justify-center text-[var(--color-accent)]"
            ...
            <div>
              <h3 className="text-lg font-bold mb-1">スコアを直接インポート</h3>
              <p className="text-sm text-[var(--color-foreground)] opacity-70">
                外部ツールのユーザーIDを入力すると、スコアデータを自動取得してインポートします
              </p>
            </div>

            <div className="w-full max-w-sm space-y-3">
              <div>
                <label htmlFor="external-user-id" className="block text-sm font-semibold mb-2 text-left">
                  ユーザーID
                </label>
                <input
                  id="external-user-id"
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && externalUserId.trim() && !directLoading) {
                      handleDirectImport();
                    }
                  }}
                  placeholder="外部ツールのユーザーIDを入力"
              ...
              <button
                ...
                disabled={directLoading || !externalUserId.trim()}
            ...
            <p className="text-xs text-[var(--color-foreground)] opacity-50 max-w-sm">
              ※ スコアデータは{" "}
              <a
                href="http://takumi3scoretool.straycubic.com/"
                target="_blank"
            ...
            {!savedExternalUserId && externalUserId.trim() && (
              <div className="flex items-center gap-2 text-xs text-[var(--color-foreground)] opacity-50 max-w-sm">
                <Settings size={12} className="flex-shrink-0" />
                <p>
                  <Link href="/settings" className="underline hover:opacity-80 font-medium">
                    設定ページ
                  </Link>
                  でユーザーIDを保存すると、次回から自動入力されます
                </p>
              </div>
            )}

            {savedExternalUserId && (
              <div className="flex items-center gap-2 text-xs text-[var(--alert-success-text)] opacity-70 max-w-sm">
                <CheckCircle size={12} className="flex-shrink-0" />
                <p>設定からユーザーIDが自動入力されています</p>
              </div>
            )}
          </div>
        </div>
        ...
        {/* 直接インポート結果表示 */}
        {directResult && renderImportResult(directResult, handleDirectReset, "もう一度インポート")}
      </section>
      ...
      <section className="space-y-6">
        ...
        >
          ...
          <div className="flex items-center gap-2 text-sm font-semibold text-[var(--color-foreground)] opacity-60 px-2
            ...
            <ChevronRight
              size={14}
              className={`transition-transform duration-200 ${csvExpanded ? "rotate-90" : ""}`}
            />
          </div>
          <div className="h-px flex-1 bg-[var(--color-header-border)]" />
        </button>
        ...
        {csvExpanded && (
          <div className="space-y-6 animate-in slide-in-from-top-2 duration-200">
            ...
            {/* CSV 結果表示 */}
            {result && renderImportResult(result, handleReset, "別のファイルをインポート")}
